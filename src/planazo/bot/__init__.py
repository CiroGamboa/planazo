...
- `commands.py` — the four command coroutines. Every reply resolves through
  `config.py` against `data/bot.yaml`, so no reply copy is spelled here.
...
message: IncomingMessage, config: BotConfig) -> None` — so the surface behind
it is swappable. `__main__.py` is the entry shim behind `python -m
planazo.bot`, and holds nothing but `sys.exit(main())`.
