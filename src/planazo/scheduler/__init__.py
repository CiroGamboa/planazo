...
service, the `format_error_entry` Rule-2 leak channel, plus the repository
primitives that read/upsert `scan_state` and the idempotent
`bootstrap_system_user` seed for the `users.telegram_user_id="system"` row
every scheduler-driven `extract_once` call attributes to.
...
preserved. The tick service (`service.py`) and CLI (`cli.py`) land in
follow-up stages of the same PR — this stage lands the data + storage
tier only.
"""
