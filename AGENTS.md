...
5. **Frameworks require an explicit runtime boundary.** LangGraph and LangChain tools are permitted only for the Recommender runtime under [ADR 0023](docs/adr/0023-langgraph-recommender-runtime.md). The graph must have an explicit typed state schema, use framework-registered tools, and preserve every existing Pydantic boundary, typed-error branch, and approval gate. CrewAI, PydanticAI, and framework adoption outside that boundary remain out of scope unless a later ADR supersedes this rule.
