...
| `AgentRunRecord` | One `agent_runs` row per completed Recommender or Extractor loop: `run_id`, `agent_kind` (`"recommender"`/`"extractor"`), `user_id`, `user_query`, `final_answer`, `stopped` (`"answered"`/`"truncated"`/`"max_steps"`), `steps_count`, `started_at`, `ended_at`. Both text fields are sanitized via `format_stored_text` (strip control chars, collapse whitespace, truncate at 2000 chars). Written best-effort at composition roots (`event_agent.run_once`, `extractor.extract_once`) alongside the JSONL sidecars — audit failures never break the primary flow (Rule 4). |
