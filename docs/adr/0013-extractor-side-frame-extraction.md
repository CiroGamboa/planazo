...
- **Passing `duration_seconds` from the adapter (Design fork 2 in the approved plan).** Rejected during implementation in favour of an internal `ffmpeg.probe` call inside `extract_reel_frames`. Reasoning: `RawPost.duration_seconds` is populated from the initial Instagram metadata fetch, which can go stale relative to the actual video content; `ffprobe` reads authoritative timing from the same MP4 bytes that get frame-sliced, closing that drift window. Trade-off: an additional subprocess spawn per reel; the `test_extract_reel_frames_probe_failure_raises_and_warns` test locks the failure mode as `FrameExtractionError` (AGENTS.md Rule 4). The zero-frames-on-unknown-duration behaviour is a deliberate change from the plan's "`[1.0, 2.0, 3.0]` safety-net fallback" — the branch prefers degrading to the thumbnail-only path cleanly over sending three frames from a possibly-broken video. The shipped helper signature is therefore `extract_reel_frames(video_url: str, *, frame_count: int = MAX_REEL_FRAMES) -> list[tuple[float, bytes]]`, not the plan's `extract_reel_frames(video_url: str, duration_seconds: float | None, *, n: int = MAX_REEL_FRAMES) -> list[bytes]`.
