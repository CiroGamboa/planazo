...
- **Superseded by:** [`0019-per-user-message-serialization.md`](0019-per-user-message-serialization.md)'s **`concurrent_updates` default and the "never crosses threads" consequence it implied, only** — see the two notes inline below (Decision → "Long polling, one connection per command"; Consequences → Negative → "Blocking work on the event-loop thread stalls every user"). Everything else here — the `UserSurface` shape, the PTB-free module boundary, session mapping, and the approval-gate threading contract off the event loop — remains accurate; `build_application` still runs the synchronous agent loop off-thread via `asyncio.to_thread`, exactly as decided below.
