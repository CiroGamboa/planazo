...
| `curator/` | Admin-scoped LLM agent — soft-deletes stale events, merges duplicates, corrects mis-classified `EventCategory` values. STRONG-tier, `max_steps=12`, daily cron via `planazo-curator --tick`. Every write reversible via `events.archived_at`. Governed by [ADR 0020](adr/0020-catalog-curator-agent.md) | `CuratorState`, `CuratorRunRecord`, `CuratorRunResult`, `run_curator_once`/`run_curator`, `build_curator_tools`, six LLM tools (three read + three write), `planazo-curator` CLI |
...
| Table | Purpose |
| --- | --- |
| `events(id, source, source_url, title, start_utc, end_utc, category, city, price_cents, geo_lat, geo_lng, confidence, extra JSON, ingested_at, event_index_in_post, source_account, venue_name, venue_address, organizer, tags JSON, description, ticket_url, image_url, language, recurring, archived_at, UNIQUE(source_url, event_index_in_post))` | The shared domain surface. `extra JSON` absorbs source-specific fields without altering the table; `tags` is a JSON array in the same TEXT-column shape. Composite `(source_url, event_index_in_post)` UNIQUE lets one Instagram post persist N distinct events (multi-event carousels) — see ADR 0012. `category` is the `EventCategory` Literal shared with `SearchIntent`; the ten domain-model columns after `event_index_in_post` land in migration 002 (see ADR 0015). `archived_at` is the curator's soft-delete column added in migration 008; `NULL` means live, non-NULL means retired — `query_events` and `get_event_by_id` filter `WHERE archived_at IS NULL` by default so the Recommender's read surface never sees curator-retired rows (see [ADR 0020](adr/0020-catalog-curator-agent.md)). Composite indexes `idx_events_city_start(city, start_utc)` and `idx_events_category_start(category, start_utc)` back the two hot Recommender filter shapes. |
...
| `agent_runs(id, run_id UNIQUE, agent_kind CHECK IN ('recommender', 'extractor', 'curator'), user_id FK, user_query, final_answer, stopped, steps_count, started_at, ended_at)` | One row per completed Recommender, Extractor, or Curator loop, written best-effort at the composition roots alongside the JSONL sidecars. `agent_kind='curator'` rows carry `user_id=NULL` (system-owned per ADR 0020). `user_query` and `final_answer` are sanitized via `observability/models.py::format_stored_text`. Backs future per-user history reads (`/find` history, #23) via the composite index `idx_agent_runs_user_started(user_id, started_at)`. Migration 010 extended the `agent_kind` CHECK to add `'curator'`. |
| `llm_decisions(id, run_id FK -> agent_runs.run_id, decision_kind CHECK IN ('save_event', 'needs_clarification', 'error', 'answered', 'archive', 'merge', 'update_category'), event_db_id FK -> events.id ON DELETE SET NULL, error_type, rationale, recorded_at)` | Zero-to-N rows per `agent_runs` — one per terminal LLM decision. Extractor emits `save_event` / `needs_clarification` / `error`; Recommender emits `answered` (or `error` on `truncated`/`max_steps`); Curator emits `archive` / `merge` (one row per archived id) / `update_category` (see [ADR 0020](adr/0020-catalog-curator-agent.md)). `rationale` is DB-inside per Rule 2 (full LLM reasoning allowed subject to `RATIONALE_CAP=500` + `format_stored_text` sanitization). Written best-effort via `observability/logging.py::LLMDecisionLogger` alongside `AgentRunLogger`; disabled together by the Recommender's `record_runs=False` seam. Indexed on `run_id` (per-run join) and `decision_kind` (corpus-analysis shape M4's ranker will read). Migration 010 extended the `decision_kind` CHECK to add the three curator kinds. |
| `curator_state(id PK CHECK (id = 1), last_run_at, last_success_at, consecutive_failures, total_archived, total_merged, total_categories_fixed)` | Singleton bookkeeping row for the catalog curator (ADR 0020). Seeded via `INSERT OR IGNORE` in migration 009 so a fresh DB reads back a defaults-only row. Read once and upserted once per `planazo-curator --tick`. `last_success_at` and `consecutive_failures=0` only advance on `stopped=answered`; every other terminal bumps `consecutive_failures++`. |
...
### 10. Curator — `src/planazo/curator/`

- Standalone CLI: `uv run planazo-curator --tick [--dry-run] [--verbose]`. Daily cron at 03:00 UTC.
- Third LLM agent alongside the Recommender and Extractor; STRONG-tier, `max_steps=12`. Admin-scoped — has write access to arbitrary `events` rows the Recommender + Extractor deliberately don't.
- Six tools ([`src/planazo/curator/tools.py`](../src/planazo/curator/tools.py)): three read (`list_stale_events`, `list_duplicate_candidates`, `list_low_confidence_events`) + three write (`archive_event`, `merge_events`, `update_event_category`).
- Every write is soft-delete or category-update — reversible via `UPDATE events SET archived_at = NULL WHERE id = <id>` (see [ADR 0020](adr/0020-catalog-curator-agent.md) §D1).
- `--dry-run` runs the LLM against the same tools but write tools return `{"status": "dry_run", ...}` — the tick's decisions are still recorded on `agent_runs` for review.
- Composition root (`curator/service.py::run_curator`) upserts the singleton `curator_state` and appends one `CuratorRunRecord` to `var/curator_runs.jsonl` per tick. `agent_kind='curator'` in `agent_runs` and `decision_kind IN ('archive', 'merge', 'update_category')` in `llm_decisions` — the monitor and any future operator dashboard pick up curator runs transparently.

Governed by **[ADR 0020 — Catalog curator agent](adr/0020-catalog-curator-agent.md)**.
...
| 0020 | [`catalog-curator-agent`](adr/0020-catalog-curator-agent.md) | Admin-scoped LLM agent (`src/planazo/curator/`) runs on daily cron at 03:00 UTC. Six tools soft-delete stale events, merge duplicates, correct mis-classified categories. STRONG tier, `max_steps=12`. Soft delete via `events.archived_at` (migration 008) — every write reversible. Singleton `curator_state` (migration 009). `AgentName` Literal + `agent_runs.agent_kind` CHECK + `llm_decisions.decision_kind` CHECK extended for `'curator'` / `'archive'` / `'merge'` / `'update_category'` (migration 010). `dry_run` is composition-time only, not an LLM-facing arg. Approval gate on writes, fuzzy dedup, operator Telegram summary deferred. |
...
| Catalog curator with LLM-driven soft-delete + merge + re-categorize | `curator/` (`run_curator`, `planazo-curator` CLI) + `events.archived_at` (migration 008) + `curator_state` (migration 009) + extended `agent_runs.agent_kind` / `llm_decisions.decision_kind` (migration 010) | `var/curator_runs.jsonl` (one `CuratorRunRecord` per tick) + `data/monitor/*.md` (grades curator ticks) + `docs/evidence/m-curator-first-tick.md` | [0020](adr/0020-catalog-curator-agent.md) |
